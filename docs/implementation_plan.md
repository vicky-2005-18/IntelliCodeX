...
#### **Week 1: Multi-Language Parsing & Tree-Sitter Integration**
* **Day 1**: Setup tree-sitter environment, dependencies (`tree-sitter`, `tree-sitter-languages`), and define language grammar loaders.
* **Day 2**: Extend [parser.py](file:///c:/Users/vikas/Downloads/Major%20project%202%202026/intellicodex/core/parser.py) to detect Python, JavaScript/TypeScript, Java, C/C++, Go, and Rust file types cleanly.
* **Day 3**: Refactor [chunker.py](file:///c:/Users/vikas/Downloads/Major%20project%202%202026/intellicodex/core/chunker.py) to extract language-agnostic AST nodes (functions, methods, classes, structs) for JS/TS and Java.
* **Day 4**: Implement Tree-Sitter AST chunking for C/C++, Go, and fallback line-block chunkers for HTML/CSS/YAML.
* **Day 5**: Wire Tree-Sitter chunker into [pipeline.py](file:///c:/Users/vikas/Downloads/Major%20project%202%202026/intellicodex/core/pipeline.py) and write comprehensive unit tests in `tests/test_chunker.py`.
* **Day 6**: Benchmark chunking granularity on sample repos of different languages; optimize chunk size bounds (100–500 tokens).
